...
export function TooltipRootProvider(props: {
...
export function useTooltip(
    ...
    const tooltipContent = (
        <TooltipContents sourceRef={sourceRef} isVisible={isVisible} position={position}>
            {contents}
        </TooltipContents>
    );
...
function TooltipContents(props: {
    ...
    useLayoutEffect(() => {
        if (!rootRef?.current || !sourceRef.current || !contentRef.current || !isVisible) return;
    ...
    if (!rootRef?.current || !isVisible) return null;
